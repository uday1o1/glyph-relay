...
| ID | State | Deliverable | Evidence |
| --- | --- | --- | --- |
| M5-L01A | `IMPLEMENTED_LOCAL` | Frozen `controller_v1` arithmetic, timing, complete degradation and restoration stack, profiles, transport eligibility, trace contract, qualification matrix, and network fixture | JSON Schema validation; semantic mutation controls; ADR 0002 live-profile correction; content-addressed protocol lock `3ab488430dca0117df8bcf93fc8e516cc39bd4bb0c39a4aecde699d7831d49c3`; `make protocol-check` |
...
| M5-L03A | `IMPLEMENTED_LOCAL` | Frozen direct-IPv4 steady and collapse matrix, strict evidence schema and validator, independent `tshark` IP-length cross-check, synchronized production controller samples, and bounded browser render and selected-candidate probe | Eight full ten-run matrix controls, six seeded gate defects, strict missing-run control, 42 Linux native and integration tests, seven sanitizer tests, 26 portable native tests, 157 Python tests, and 41 TypeScript and JavaScript tests; commit `d81c8117b2b59c7124818fe7850aef629a7a04cf`; `make linux-cpu-check`; `make check` |
| M5-L03B | `PENDING_LOCAL` | Linux network-namespace topology, `tc netem` acquisition runner, bounded signaling proxy, `tshark` capture orchestration, evidence assembly and cleanup, seeded topology tests, and consolidated target phase | None |
...
| `DEFERRED_HARDWARE` | Stable live link sustains at least 24 compositor fps, 95 percent delivery, 720p30, and at most 250 ms P95 latency under ADR 0002 | None |
